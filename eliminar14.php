...
$eliminar = "DELETE FROM cargos WHERE id = $id_eliminar";
$eliminar_result = $conn->query($eliminar) or die ("database error:".$conn->error);
$eliminar2 = "DELETE FROM trabaja WHERE cargo_id = $id_eliminar";
$eliminar2_result = $conn->query($eliminar2) or die ("database error:".$conn->error);
$eliminar = "DELETE FROM superiores_id WHERE cargo_id = $id_eliminar OR cargo2_id = $id_eliminar";
$eliminar_result = $conn->query($eliminar) or die ("database error:".$conn->error);
$eliminar = "DELETE FROM opinantes WHERE cargo_id = $id_eliminar OR cargo_sup = $id_eliminar";
$eliminar_result = $conn->query($eliminar) or die ("database error:".$conn->error);
$eliminar = "DELETE FROM indicador_cargos WHERE cargo_id = $id_eliminar";
$eliminar_result = $conn->query($eliminar) or die ("database error:".$conn->error);
$evalcomp = "SELECT id FROM evaluaciones_comp
										WHERE cargo_id = $id_eliminar
										   OR cargo_sup = $id_eliminar";
